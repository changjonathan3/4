...
public class DijkstraSP {
    ...
    public DijkstraSP(EdgeWeightedDigraph G, int s) {
        ...
        for (DirectedEdge e : G.edges()) {
            if (e.getBandwidth() < 0)
                throw new IllegalArgumentException("edge " + e + " has negative bandwidth");
            if (e.getLength() < 0)
                throw new IllegalArgumentException("edge " + e + " has negative length");
            if (e.getTimeToTravel() < 0)
                throw new IllegalArgumentException("edge " + e + " has negative time to travel");
        }
    ...
    private void relax(DirectedEdge e) {
        int v = e.from(), w = e.to();
        if (distTo[w] > distTo[v] + e.getTimeToTravel()) {
            distTo[w] = distTo[v] + e.getTimeToTravel();
            edgeTo[w] = e;
    ...
    private boolean check(EdgeWeightedDigraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (DirectedEdge e : G.adj(v)) {
                int w = e.to();
                if (distTo[v] + e.getTimeToTravel() < distTo[w]) {
